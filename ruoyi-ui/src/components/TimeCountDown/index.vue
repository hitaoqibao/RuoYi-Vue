<template>
  <span>
    <slot :day="day" :hour="hour" :minute="minute" :second="second">
      <template v-if="!ignoreDay">{{ day }}:</template>
      {{ hour }}:{{ minute }}:{{ second }}
    </slot>
  </span>
</template>
...
export default {
  ...
  methods: {
    render() {
      ...
      if (this.ignoreDay) {
        this.hour = FORMAT_NUMBER(
          Math.floor(restTime / (60 * 60)),
          this.completeZero
        );
      } else {
        this.day = FORMAT_NUMBER(
    ...
    getRestTime() {
      // 获取剩余时间（秒）
      return Math.max(
        Math.round((this.deadlineTimestamp - Date.now()) / 1000),
        0
      );
    },
  },
