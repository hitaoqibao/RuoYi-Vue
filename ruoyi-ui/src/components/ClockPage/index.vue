...
export default {
  ...
  props: {
    // canvas 画布的宽度，默认为画布容器的宽度
    width: {
      default: ""
    },
    //canvas 画布的高度，默认与画布的宽度一致
    height: {
      default: ""
    },
    //自适应，如果容器小于时钟的设定大小，时钟根据容器的大小自行调整
    adaptive: {
      type: Boolean,
      default: true
    },
    //时区，如时区为 UTC+8，则 timezone=8
    timezone: {
      default: ""
    },
    //时间
    ...
    //minute	Object		分针
    //hour	Object		时针
    //注：在 needle 中，secone、minute、hour 分别为一个对象，每个对象中的属性名和代表的意义是一样的
    // 属性	类型	默认值	说明
    // length	Number	second : 100
    // minute : 110
    // hour : 70	针的长度
    // lineWidth	String	second : 3
    //minute : 4
    //hour : 4	针的宽度（粗细）
    //longOut	Number	second : 5
    //minute : 5
    //hour : 5	超出圆心的长度
    //color	String	second : #aaa
    //minute : #999
    //hour : #888	针的颜色
    needle: {
      type: Object,
  ...
  methods: {
    ...
    drawBorder() {
      ...
      if (this.drawOption.border.type == "rectangle") {
        this.context.rect(
          -this.drawOption.border.width / 2,
          -this.drawOption.border.height / 2,
          this.drawOption.border.width,
          this.drawOption.border.height
        );
      } else {
        this.context.arc(
          0,
    ...
    drawDial() {
      ...
      var degM = 0;
      var distance = this.drawOption.dial.distance
        ? this.drawOption.dial.distance + this.drawOption.border.lineWidth / 2
        : this.drawOption.border.lineWidth / 2;
      if (this.drawOption.dial.distance == 0)
        distance = this.drawOption.border.lineWidth / 2;
      var begainPosition = {
        x: 0,
      ...
      for (let i = 0; i < 60; i++) {
        ...
        if (i % 5 == 0) {
          this.line(
            begainPosition,
            degM,
            this.drawOption.dial.maxLength - begainPosition.y,
            this.drawOption.dial.color,
            this.drawOption.dial.maxWidth
          );
        } else {
          this.line(
            begainPosition,
            degM,
            this.drawOption.dial.minLength - begainPosition.y,
            this.drawOption.dial.color,
            this.drawOption.dial.minWidth
          );
        }
        this.context.restore();
      }
    ...
    drawNumber() {
      ...
      this.context.font =
        this.drawOption.number.fontWeight +
        " " +
        this.drawOption.number.fontSize +
        " " +
        this.drawOption.number.fontFamily;
      ...
      for (let i = 0; i < num.length; i++) {
        this.context.fillText(
          num[i],
          this.drawOption.number.radius * Math.cos(ar * (i + 1)),
          this.drawOption.number.radius * Math.sin(ar * (i + 1))
        );
      }
      this.context.restore();
    },
    ...
    drawNeedle() {
      ...
      this.line(
        ...
        h,
        this.drawOption.needle.hour.length,
        this.drawOption.needle.hour.color,
        this.drawOption.needle.hour.lineWidth
      ); //时针
      this.line(
        {
          x: 0,
        ...
        m,
        this.drawOption.needle.minute.length,
        this.drawOption.needle.minute.color,
        this.drawOption.needle.minute.lineWidth
      ); //分针
      this.line(
        {
          x: 0,
        ...
        s,
        this.drawOption.needle.second.length,
        this.drawOption.needle.second.color,
        this.drawOption.needle.second.lineWidth
      ); //秒针
      this.context.restore();
    },
    //以时区确定时间
    selectTimezone() {
      var d = this.drawOption.time.isStatic
        ? new Date(this.drawOption.time.timestamp)
        : new Date();
      //得到1970年一月一日到现在的秒数
      var len = d.getTime();
      //确定时区
